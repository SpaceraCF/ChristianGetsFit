/**
 * Safe seed: only runs if the exercises table is empty.
 * Called automatically during build to ensure exercises exist after deploy.
 */
...
async function main() {
  ...
  if (count > 0) {
    console.log(`[seed-safe] ${count} exercises already exist — skipping.`);
    return;
  }

  console.log("[seed-safe] No exercises found — seeding...");
  ...
  for (const ex of warmUps) await prisma.exercise.create({ data: ex });

  // Workout A - Push
  await prisma.exercise.create({ data: { name: "Smith Machine Bench Press", muscleGroup: "chest", equipment: "smith_machine", instructions: "Lie on bench, grip bar slightly wider than shoulders. Lower to chest, press up.", baseWeightPercent: 0.3, weightIncrementKg: 2.5, substituteExerciseIds: [], warmUpOrder: null, workoutType: "A", orderInWorkout: 0, sets: 3, repsMin: 8, repsMax: 12, restSecs: 90, isWarmUp: false, injuryAreasToSkip: ["shoulder", "wrist"] } });
  await prisma.exercise.create({ data: { name: "Smith Machine Incline Bench Press", muscleGroup: "chest", equipment: "smith_machine", instructions: "Set bench to 30-45°. Same as bench press, targets upper chest.", baseWeightPercent: 0.25, weightIncrementKg: 2.5, substituteExerciseIds: [], warmUpOrder: null, workoutType: "A", orderInWorkout: 1, sets: 3, repsMin: 8, repsMax: 12, restSecs: 90, isWarmUp: false, injuryAreasToSkip: ["shoulder", "wrist"] } });
  const dbSP = await prisma.exercise.create({ data: { name: "Dumbbell Shoulder Press", muscleGroup: "shoulders", equipment: "dumbbell", instructions: "Seated or standing. Press dumbbells overhead from shoulder height.", baseWeightPercent: 0.1, weightIncrementKg: 1, substituteExerciseIds: [], warmUpOrder: null, workoutType: "A", orderInWorkout: 2, sets: 3, repsMin: 8, repsMax: 12, restSecs: 90, isWarmUp: false, injuryAreasToSkip: ["shoulder"] } });
  const arnP = await prisma.exercise.create({ data: { name: "Arnold Press", muscleGroup: "shoulders", equipment: "dumbbell", instructions: "Start palms in, rotate out as you press. Lighter than regular press.", baseWeightPercent: 0.08, weightIncrementKg: 1, substituteExerciseIds: [], warmUpOrder: null, workoutType: "A", orderInWorkout: 3, sets: 3, repsMin: 8, repsMax: 12, restSecs: 90, isWarmUp: false, injuryAreasToSkip: ["shoulder"] } });
  await prisma.exercise.create({ data: { name: "Dumbbell Tricep Extension", muscleGroup: "triceps", equipment: "dumbbell", instructions: "One arm or two. Lower dumbbell behind head, extend up.", baseWeightPercent: 0.06, weightIncrementKg: 1, substituteExerciseIds: [], warmUpOrder: null, workoutType: "A", orderInWorkout: 4, sets: 3, repsMin: 8, repsMax: 12, restSecs: 60, isWarmUp: false, injuryAreasToSkip: ["elbow", "wrist"] } });
  await prisma.exercise.create({ data: { name: "Lateral Raise", muscleGroup: "shoulders", equipment: "dumbbell", instructions: "Arms at sides, raise to shoulder height. Control the negative.", baseWeightPercent: 0.04, weightIncrementKg: 1, substituteExerciseIds: [], warmUpOrder: null, workoutType: "A", orderInWorkout: 5, sets: 3, repsMin: 10, repsMax: 12, restSecs: 60, isWarmUp: false, injuryAreasToSkip: ["shoulder"] } });

  // Workout B - Pull
  const smR = await prisma.exercise.create({ data: { name: "Smith Machine Bent-Over Row", muscleGroup: "back", equipment: "smith_machine", instructions: "Hinge at hips, pull bar to lower chest. Squeeze shoulder blades.", baseWeightPercent: 0.25, weightIncrementKg: 2.5, substituteExerciseIds: [], warmUpOrder: null, workoutType: "B", orderInWorkout: 0, sets: 3, repsMin: 8, repsMax: 12, restSecs: 90, isWarmUp: false, injuryAreasToSkip: ["back"] } });
  const csR = await prisma.exercise.create({ data: { name: "Chest-Supported Dumbbell Row", muscleGroup: "back", equipment: "dumbbell", instructions: "Lie chest on incline bench, row dumbbells to hip. Back-supported.", baseWeightPercent: 0.12, weightIncrementKg: 1, substituteExerciseIds: [], warmUpOrder: null, workoutType: "B", orderInWorkout: 1, sets: 3, repsMin: 8, repsMax: 12, restSecs: 90, isWarmUp: false, injuryAreasToSkip: [] } });
  await prisma.exercise.create({ data: { name: "Dumbbell Bicep Curl", muscleGroup: "biceps", equipment: "dumbbell", instructions: "Arms at sides, curl up. Keep elbows still.", baseWeightPercent: 0.06, weightIncrementKg: 1, substituteExerciseIds: [], warmUpOrder: null, workoutType: "B", orderInWorkout: 2, sets: 3, repsMin: 8, repsMax: 12, restSecs: 60, isWarmUp: false, injuryAreasToSkip: ["elbow", "wrist"] } });
  await prisma.exercise.create({ data: { name: "Smith Machine Shrug", muscleGroup: "traps", equipment: "smith_machine", instructions: "Bar at arms length. Shrug shoulders up and back.", baseWeightPercent: 0.25, weightIncrementKg: 2.5, substituteExerciseIds: [], warmUpOrder: null, workoutType: "B", orderInWorkout: 3, sets: 3, repsMin: 10, repsMax: 12, restSecs: 60, isWarmUp: false, injuryAreasToSkip: ["neck"] } });
  await prisma.exercise.create({ data: { name: "Hammer Curl", muscleGroup: "biceps", equipment: "dumbbell", instructions: "Neutral grip (palms in). Curl both arms. Easier on wrists.", baseWeightPercent: 0.06, weightIncrementKg: 1, substituteExerciseIds: [], warmUpOrder: null, workoutType: "B", orderInWorkout: 4, sets: 3, repsMin: 8, repsMax: 12, restSecs: 60, isWarmUp: false, injuryAreasToSkip: ["elbow", "wrist"] } });
  await prisma.exercise.create({ data: { name: "Band or Cable Face Pull", muscleGroup: "rear_delts", equipment: "dumbbell", instructions: "If no cable: bent-over reverse fly with light dumbbells. Pull to face level.", baseWeightPercent: 0.04, weightIncrementKg: 1, substituteExerciseIds: [], warmUpOrder: null, workoutType: "B", orderInWorkout: 5, sets: 3, repsMin: 12, repsMax: 15, restSecs: 60, isWarmUp: false, injuryAreasToSkip: ["shoulder"] } });

  // Workout C - Legs + Core
  await prisma.exercise.create({ data: { name: "Smith Machine Squat", muscleGroup: "legs", equipment: "smith_machine", instructions: "Bar on upper back. Squat to parallel or below. Knees track over toes.", baseWeightPercent: 0.4, weightIncrementKg: 2.5, substituteExerciseIds: [], warmUpOrder: null, workoutType: "C", orderInWorkout: 0, sets: 3, repsMin: 8, repsMax: 12, restSecs: 90, isWarmUp: false, injuryAreasToSkip: ["knee", "back"] } });
  await prisma.exercise.create({ data: { name: "Smith Machine Leg Press (or Hack Squat)", muscleGroup: "legs", equipment: "smith_machine", instructions: "Feet on platform, lower and press. Back supported.", baseWeightPercent: 0.5, weightIncrementKg: 2.5, substituteExerciseIds: [], warmUpOrder: null, workoutType: "C", orderInWorkout: 1, sets: 3, repsMin: 8, repsMax: 12, restSecs: 90, isWarmUp: false, injuryAreasToSkip: ["knee", "back"] } });
  const dbL = await prisma.exercise.create({ data: { name: "Dumbbell Walking Lunge", muscleGroup: "legs", equipment: "dumbbell", instructions: "Hold dumbbells at sides. Lunge forward, alternate legs.", baseWeightPercent: 0.07, weightIncrementKg: 1, substituteExerciseIds: [], warmUpOrder: null, workoutType: "C", orderInWorkout: 2, sets: 3, repsMin: 8, repsMax: 10, restSecs: 90, isWarmUp: false, injuryAreasToSkip: ["knee", "hip"] } });
  const stU = await prisma.exercise.create({ data: { name: "Dumbbell Step-Up", muscleGroup: "legs", equipment: "dumbbell", instructions: "Step onto bench or box with one leg. Drive up. Alternate.", baseWeightPercent: 0.07, weightIncrementKg: 1, substituteExerciseIds: [], warmUpOrder: null, workoutType: "C", orderInWorkout: 3, sets: 3, repsMin: 8, repsMax: 10, restSecs: 60, isWarmUp: false, injuryAreasToSkip: ["knee", "hip"] } });
  ...
  // Substitutions
  await prisma.exercise.update({ where: { id: dbSP.id }, data: { substituteExerciseIds: [arnP.id] } });
  await prisma.exercise.update({ where: { id: arnP.id }, data: { substituteExerciseIds: [dbSP.id] } });
  await prisma.exercise.update({ where: { id: smR.id }, data: { substituteExerciseIds: [csR.id] } });
  await prisma.exercise.update({ where: { id: csR.id }, data: { substituteExerciseIds: [smR.id] } });
  await prisma.exercise.update({ where: { id: dbL.id }, data: { substituteExerciseIds: [stU.id] } });
  await prisma.exercise.update({ where: { id: stU.id }, data: { substituteExerciseIds: [dbL.id] } });

  const total = await prisma.exercise.count();
  console.log(`[seed-safe] Seeded ${total} exercises.`);
}
